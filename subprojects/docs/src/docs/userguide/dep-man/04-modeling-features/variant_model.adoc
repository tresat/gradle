...
Variants without attributes cannot participate in variant selection and are only accessible through explicit configuration selection or legacy resolution mechanisms.
====
...
It also accepts the `--all` flag to include information about legacy and deprecated configurations, as well as variants without attributes.

NOTE: Variants without attributes are marked with `(n)` in the report and cannot be selected during variant-aware dependency resolution.
