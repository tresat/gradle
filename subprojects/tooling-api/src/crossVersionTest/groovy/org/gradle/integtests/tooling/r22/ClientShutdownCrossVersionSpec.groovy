...
class ClientShutdownCrossVersionSpec extends ToolingApiSpecification {
    ...
    private List<String> getBuildJvmArguments() {
        return ImmutableList<String>.builder().add("-Djava.io.tmpdir=${buildContext.getTmpDir().absolutePath}").addAll(JVM_OPTS).build()
    }
    ...
    def "cleans up idle daemons when tooling API session is shutdown"() {
        withConnection { connection ->
            connection.model(GradleBuild).addJvmArguments(getBuildJvmArguments()).get()
        }
